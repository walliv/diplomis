...
\begin{frame}
	...
	\begin{itemize}
			...
			\item DMA Iuventus architecture
...
\begin{frame}
	...
	\begin{itemize}
		\item Direct access to persistent data from accelerator
		\item Conventional CPU-based copies introduce an overhead
		\item Host memory used for staging (double copying)
		\item With peer-to-peer transfers, the NVMe can be directly accessed
		\item Linux user friendly (standardized filesystem)
	\end{itemize}
	...
	\begin{columns}[T] 					% columns environment, top placement
		\begin{column}{0.6\textwidth}	% first column
			\begin{itemize}
				...
				\item Memory-mapped communication
	...
	\begin{columns}[T] 					% columns environment, top placement
		\begin{column}{0.5\textwidth}	% first column
			\begin{itemize}
				...
				\item Address-based routing (for our case :)
				\item Host bypass possible if platform permits P2P
	...
	\begin{columns}[T] 					% columns environment, top placement
		\begin{column}{0.5\textwidth}	% first column
			\begin{itemize}
				...
				\item Commands are fetched by controller over PCIe
				\item Pointers synchronize queue state (doorbells)
				\item Same protocol used by host and FPGA requesters
			\end{itemize}
		\end{column}
		%
		\begin{column}{0.5\textwidth}		% second column
	...
	\begin{columns}[T] 					% columns environment, top placement
		\begin{column}{0.5\textwidth}	% first column
			\begin{itemize}
				...
				\item Core component: DMA Iuventus IP
				\item Accelerator BARs hold queues and data buffers
				\item SPDK configures NVMe and queue resources
				\item Linux ublk exposes mountable block device
				\item Shared CPU-FPGA access to one filesystem
			\end{itemize}
		\end{column}
		%
		\begin{column}{0.5\textwidth}		% second column
...
\begin{frame}
	\vspace{1cm}
	\textbf{Next step:} live demo of end-to-end shared read/write access.
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Shared CPU-FPGA filesystem access over NVMe was demonstrated
		\item PCIe peer-to-peer path avoids mandatory host-memory staging
