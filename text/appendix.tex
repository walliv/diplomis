...
The synthesis reports are available in \verb|*_synth.drc|, \verb|*_synth.tim| and
\verb|*_synth.util| files whereas the reports from implementation in \verb|*_par.drc|,
\verb|*_par.tim| and \verb|*_par.util| files. The generated DeviceTree can be seen in
\verb|*.netcope_tmp/DeviceTree.dts| file. There is the Vivado-specific bitstream as the \verb|*.bit|
file and its NDK-FPGA-specific option in the \verb|*.nfw| file. The \verb|*.nfw| file is basically
an archive containing the \verb|*.bit| bitstream and the DeviceTree file used by the NDK-SW
programming tool for protection to check that the bitstream gets programmed into a correct hardware.
