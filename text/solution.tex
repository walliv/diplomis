...
both the communication topology and the physical properties of the interconnect\cite{pcie_book}.
...
\footnotetext{
    ...
    can be present. The Function is basically and addressable entity in the configuration space where
...
Multiple Root Ports or Downstream ports enable direct \ac{D2D} communication, as shown
...
in~\cref{tab:res_consumption}.

\begin{table}[t]
    \centering
    \caption[PCIe throughput based on its versions]{PCIe throughput based on its versions. The
    throughput is calculated without the overhead of the Physical layer}
    \label{tab:res_consumption}
    \begin{tabular}{cccccccc}
    \toprule
    \multirow{2}{*}{Version} & \multicolumn{2}{c}{Line code} & \multicolumn{5}{c}{Link throughput [GBps]} \\
...
by introducing external power supply attachments. The most common ATX connector adds another 75~W
for its 6-pin configuration or 150~W for the 8-pin configuration\cite{atx_spec}.
...
on~\cref{fig:pcie_diff_link_widths} where the M.2 SSD needs to communicate with the processor
(where Root Ports can usually utilize up to 16 lanes) or with a \ac{NIC} which supports up to 16
lanes. Other than link's lane count and transfer rate, other configured parameters include lane
polarity and lane reversal. Since the training and the negotiated link parameters are tha matter of
the physical layer, the link properties remain transparent for the user.
...
to introduce \ac{DMA} that allows to copy large amounts of data without the host CPU facilitating
...
Root Ports) (source to spec). The routing of TLP differs based on its type with \acp{MWr} and
...
through the \ac{rcpx} even if the devices are connected to the single Switch (this is shown
in~\cref{fig:pcie_acr}).  Otherwise, all devices under each Root Port are put to a common IOMMU
group which does not play well with the need to decide isolation on per-device basis.
...
serial chip \verb+vu57p+)\cite{alveo_u55c}. The available integrated core called \emph{PCIE4C}
allows to configure the interface to run on Gen3 transfer rat on 16 lanes (Gen3x16) or Gen4 transfer
rate on 8 lanes (Gen4x8) providing the same throughput but reducing logic utilization. Since two of
...
The mostly used used commands in this thesis on runtime are read and write from/to the \ac{NVM} that
