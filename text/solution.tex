...
The value that is considered valid is flipped upon every \ac{CQTDBL} rollover, starting as log.\ 1
for the initial pass through queue (i.e.\ after queue initialization), then flipping to 0 when the
pointer rolls back to 0 and so on.
...
the master about the status of a command. The template of every entry is shown in Fig. XXX.
...
% Multiple masters x multiple NVMe controllers
