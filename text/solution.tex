...
\begin{itemize}
        \item Shared-bus topology that, in spite of its simplicity, was rendered unscalable since with
        ...
        these two waves, the signal reaches the required voltage level for the digital circuits\cite{pcie_book}.
\end{itemize}
...
Switches, the number of devices can be increased arbitrarily, making PCIe a switched network\cite{pcie_spec}.
Although the PCIe specification introduces many advanced features, this thesis
presents mostly the necessary ones for its purposes.

\begin{figure}[!h]
    \centering
    \includegraphics[scale=1.2]{pict/host_topology-raw_topology}
    \caption[Example of PCIe topology]{Example of a topology using PCIe interconnect with its designated elements}
...
\begin{figure}[!h]
    ...
    \caption[Examples of PCIe transports]{Examples of the transports occurring in the PCIe domain. Each device can be both
      Requester (i.e.\ dispatching a request) and a Completer (i.e.\ receiving a request) at the
      same time.}
...
either contain 1, 2, 4, 8 or 16 lanes. The communication speeds are shown in~\cref{tab:res_consumption}.

\begin{table}[t]
    \centering
    \caption[PCIe throughput based on its versions]{PCIe throughput based on its versions. The throughput
    is calculated without the overhead of the Physical layer}
...
\begin{figure}[!h]
    ...
    \caption[Various types of PCIe CEM connectors]{Different types of CEM PCIe connectors supporting 4,
    16, 1 or 16 lanes (looking from the top). The bottom connector belongs to the old PCI
...
\begin{figure}[!h]
    ...
    \caption[PCIe topology with different link widths]{Topology with multiple interconnected devices where each supports different link
      width.}
...
serial chip \verb+vu57p+)\cite{alveo_u55c}. The available integrated core called \emph{PCIE4C} allows to configure
the interface to run on Gen3 transfer rat on 16 lanes (Gen3x16) or Gen4 transfer rate on 8
lanes (Gen4x8) providing the same throughput but reducing logic utilization. Since two of
...
initialized system with a 4-core processor and an NVMe device is shown
...
\begin{figure}[!h]
    ...
    \caption[Initialized system with one NVMe device and a CPU]{Initialized system with an NVMe device
        communicating with the host CPU. The indexing of the IO adheres to the standard since the
        index 0 is reserved for the Admin queue-pair.}
...
\begin{figure}[!h]
    ...
    \caption[Format of Submission Queue Entry]{Format of Submission Queue Entry. The greyed-out fields are either reserved or
      unused within this thesis.}
    \label{fig:nvme_sqe}
\end{figure}
...
\begin{figure}[!h]
    ...
    \caption[Format of Completion Queue Entry]{Format of Completion Queue Entry. The greyed-out fields are either reserved or
      unused within this thesis.}
    \label{fig:nvme_cqe}
\end{figure}
...
the \ac{CID} field is important so the submitter can bound a completion to a dispatched command.
The order of execution can therefore be derived from the order in which the completions for multiple
...
topology\footnotemark\@ as seen in~\cref{fig:nvme_multipath}. This feature that allows to access one NVMe device from
multiple masters is sometimes called \emph{multi-pathing}.
